...
BACKUP_FILE="$HOME/provision/artifactory-jcr/backup.zip"
...
function login() {
  ...
  LOGIN_URL="$ARTIFACTORY_JCR_URL/artifactory/ui/auth/login"
  ...
  if [ $RESPONSE_CODE -eq 200 ]; then
    printf "${GREEN}Artifactory JCR login successful${NC}"
  else
    printf "${RED}There has been an error while loging into Artifactory JCR${NC}"
    exit 1
  fi
}
...
function restoreBackup() {
  ...
  if [ $RESPONSE_CODE -eq 200 ]; then
    printf "${GREEN}Artifactory JCR has been configured successfully!${NC}"
  else
    printf "${RED}There has been an error while restoring artifactory JCR backup. Code: $RESPONSE_CODE ${NC}"
  fi
  rm $COOKIES_FILE
}
